...
export type UnwrapDerivedOrStore<T> =
  T extends Derived<infer InnerD> ? InnerD : T extends Store<infer InnerS> ? InnerS : never;

type UnwrapReadonlyDerivedOrStoreArray<TArr extends ReadonlyArray<Derived<any> | Store<any>>> =
...
> {
  ...
  options: DerivedOptions<State, TArr>;
  ...
  constructor(options: DerivedOptions<State, TArr>) {
  ...
  registerOnGraph(deps: ReadonlyArray<Derived<any> | Store<any>> = this.options.deps) {
  ...
  unregisterFromGraph(deps: ReadonlyArray<Derived<any> | Store<any>> = this.options.deps) {
    for (const dep of deps) {
      if (dep instanceof Derived) {
